...
data PolyNum = PN { num:: Integer }

instance Eq PolyNum where
        (PN { num=a1}) == (PN {num=a2}) = a1 == a2

instance Ord PolyNum where
        (PN { num=a}) <= (PN {num=b}) = (snd2Num a) == (fst2Num b)

p61e = solveProblem [Square,Pentagonal]
p61  = solveProblem [Square, Pentagonal, Hexagonal, Heptagonal, Octagonal]

solveProblem polytypes = do
	tn <- numList triangleNum

	p0 <- polytypes
	m0 <- nextPolyNums' tn p0

	p1 <- (delete p0 polytypes)
	m1 <- nextPolyNums' m0 p1

	p2 <- (delete p1 polytypes)
	m2 <- nextPolyNums' m1 p2 

	p3 <- (delete p2 polytypes)
	m3 <- nextPolyNums' m2 p3 

	p4 <- (delete p3 polytypes)
	m4 <- nextPolyNums' m3 p4 
	guard ( m4 `l2EQf2` tn)
	return [tn:m0:m1:m2:m3:m4]
	
	
	--tn <- numList triangleNum
	--ps <- matchingPolySet tn [Square, Pentagonal]
	--return (tn,ps )
	--guard ( (last ps) `l2EQf2` tn)
	--guard ( (liftM last ps) `l2EQf2` tn)
	--(tn:ps)

--matchingPolySet :: Int -> [PolygonalType] -> [Int]
--matchingPolySet n pts = foldl step [n] pts
--	where step ns pt = foldl1 step' [n] $ nextPolyNums' n pt 
--		where step' sv pns =   nextPolyNums' pns (delete pt pts)

--	where step ns pt = nextPolyNums' n pt

--foo	     :: Int -> [PolygonalType] -> [Int] -> [Int] 
--foo _ []  rs = reverse rs
--foo n pts rs = do (pt',n') <- nextPolyNums n pts
--	          foo n' (pts \\ [pt']) (n':rs)
...
nextPolyNums' :: Int -> PolygonalType -> [Int]
nextPolyNums' n pt = do let n' = snd2Num n
			n'' <- matchingNums n pt
			guard((snd2Num n'') > 9)
			return n'' 


nextPolyNums :: Int -> [PolygonalType] -> [(PolygonalType,Int)]
nextPolyNums n pts = do let n' = snd2Num n
			guard(n' > 10)
			mpn@(_,n'') <- getMatchingPolyNums n' pts
			guard((snd2Num n'') > 10)
			return mpn


--matchingNum :: Int -> [PolygonalType] -> State 

--findingCyclic :: CyclicState Int
--findingCyclic = do c@(v,_) <- get
		 

--matchingNum :: Int -> PolyState Int
--matchingNum n = do (st,v) <- get
--		   return v 
 
matchCyc n xs = do x <- xs
		   return x

getMatchingPolyNums :: Int -> [PolygonalType] -> [(PolygonalType,Int)]
getMatchingPolyNums  n ts = do (t,f) <- funcLookup ts
	 		       nn <- f n
			       return (t,nn)


funcLookup :: [PolygonalType] -> [(PolygonalType,Int->[Int])]
funcLookup ts = liftM lf ts
	where lf a = (a, makePolyNums (maybe (0 +) (id) (lookup a polygonalFuncs2Types)))
	     

 
matchingNums :: Int -> PolygonalType -> [Int]
matchingNums n pt
 | (l2n > 9) = makePolyNums (maybe (0 +) (id) (lookup pt polygonalFuncs2Types)) l2n
 | otherwise = []
	where l2n = snd2Num n
	
		    

makePolyNums :: (Int -> Int) -> Int -> [Int]
makePolyNums f b = takeWhile(< ub) $ dropWhile (< lb ) $ map f [1 ..]
	where ub = lb + 100
	      lb = b * 100


type PolygonalState = State [(PolygonalType,Bool)]
...
polygonalTypes = [Triangle, Square, Pentagonal, Hexagonal, Heptagonal, Octagonal]
...
l2EQf2 a b = (snd2Num a) == (fst2Num b)

--p61 :: Int
p61'0 = sum $ take 6 dowork
	where dowork = do
		  tn <- numList' triangleNum 
...
p61' = do 
   let polySet = [Square, Pentagonal, Hexagonal, Heptagonal, Octagonal]
   triNum <- numList triangleNum
   ms <- polyMatches triNum polySet
   guard ( (last ms) `l2EQf2` triNum)
   (triNum : ms)
...
polyMatches :: Int -> [PolygonalType] -> [[Int]]
polyMatches _ []  = return []
polyMatches n pts = do
   		p   <- pts
		m   <- matchingNums n p
		nxt <- polyMatches m (delete p pts)
		return (m : nxt)
...
p61e_waterfall = do
   ...
   triNum <- numList triangleNum
   p      <- polySet
   m      <- matchingNums triNum p 
   p'     <- (delete p polySet)
   m'     <- matchingNums m p'
   guard ( m' `l2EQf2` triNum)
   [triNum,m,m']
